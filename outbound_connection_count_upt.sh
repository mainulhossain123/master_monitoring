#!/bin/bash
#
# This script monitors outbound connections for a .NET Core application.
# If the number of connections exceeds a predefined threshold, it generates a memory dump, profiler trace, or both for diagnostics.
#
...
function usage()
{
    ...
    echo "-t <threshold> Threshold of outbound connections to trigger dump/trace. Default: 100"
...
echo "Starting outbound connection monitoring with $diagnostic_option collection enabled..."
...
output_dir="outbound-logs-$instance"
mkdir -p "$output_dir"
output_file="$output_dir/outbound_log_$(date '+%Y%m%d_%H').log"
...
while true; do
    ...
    echo "$(date '+%Y-%m-%d %H:%M:%S'): Outbound connection count: $connection_count" >> "$output_file"
